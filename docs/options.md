...
| `displayMilliseconds` | boolean | `false` | Indicates if milliseconds should be included in time displays, e.g. `00:00:000` vs `00:00`. Use the `formatTime` option if you need more control of the time format. |
| `formatTime` | function | undefined | Use a custom time format function, for example: ```(seconds, guide) => `test:${seconds}:${guide}` ``` |
